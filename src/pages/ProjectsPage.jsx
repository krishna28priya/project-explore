...
const ProjectsPage = () => {
  ...
  const domain = useMemo(() => {
    return projectsData.domains.find(d => d.id === parseInt(domainId));
  }, [domainId]);

  const filteredProjects = useMemo(() => {
    if (!domain) return [];
    
    return domain.projects.filter(project => {
  ...
  }, [domain, searchTerm, selectedLevel]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-xl shadow-md p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 text-center">
            <div>
              <div className="text-3xl font-bold text-purple-600">{domain.projects.length}</div>
              <div className="text-gray-600">Total Projects</div>
            </div>
            <div>
              <div className="text-3xl font-bold text-blue-500">
                {domain.projects.filter(p => p.level.toLowerCase() === 'basic').length}
              </div>
              <div className="text-gray-600">Basic Projects</div>
            </div>
            <div>
              <div className="text-3xl font-bold text-green-500">
                {domain.projects.filter(p => p.level.toLowerCase() === 'intermediate').length}
              </div>
              <div className="text-gray-600">Intermediate Projects</div>
            </div>
            <div>
              <div className="text-3xl font-bold text-yellow-500">
                {domain.projects.filter(p => p.level.toLowerCase() === 'advanced').length}
              </div>
              <div className="text-gray-600">Advanced Projects</div>
            </div>
            <div>
              <Link to={`/learn/${domainId}`}>
                <button className="w-full bg-gradient-to-r from-purple-600 to-blue-500 text-white py-2 rounded-lg hover:opacity-90 transition-opacity font-bold">
                  Learn Domain
                </button>
              </Link>
            </div>
        ...
        <div className="mb-6">
          <p className="text-gray-600">
            Showing <span className="font-bold">{filteredProjects.length}</span> of <span className="font-bold">{domain.projects.length}</span> projects
          </p>
        </div>
        ...
        {!currentUser && filteredProjects.length > 0 && (
          <div className="mt-16 bg-gradient-to-r from-purple-600 to-blue-500 rounded-2xl shadow-xl p-8 text-center">
            ...
            <p className="text-xl text-purple-100 mb-8 max-w-2xl mx-auto">
              Join now to track your progress, save your favorite projects, and build your professional portfolio.
            </p>
